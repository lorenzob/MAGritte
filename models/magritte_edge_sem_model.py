...
class MAGritteEdgeSemModel(BaseModel):
    """ This class implements the MAGritteEdgeSem model, for learning a mapping from input images to output images given paired data.
    ...
    def modify_commandline_options(parser, is_train=True):
        ...
        if is_train:
            ...
            parser.add_argument('--lambda_S', type=float, default=10.0, help='weight for S loss')
            parser.add_argument('--lambda_Gb_GAN', type=float, default=0.0, help='weight for Gb_GAN loss')

        return parser

    def __init__(self, opt):
        """Initialize the MAGritteEdgeSemModel class.
        ...
        self.loss_names = ['Gb_GAN', 'Gb_L1', 'Gb_M', 'Db_real', 'Db_fake', 'Gc_GAN', 'Gc_L1', 'Gc', 'Dc_fake', 'Dc_real', 'Gb_S']
        # specify the images you want to save/display. The training/test scripts will call <BaseModel.get_current_visuals>
        self.visual_names = ['real_A', 'fake_B', 'fake_B_RGB', 'fake_B_A', 'real_B', 'fake_CA_fake_vis', 'fake_CA_fake_edge_vis', 'fake_CB_real_B_fake_vis', 'fake_CB_real_B_fake_vis', 'fake_CA_class', 'fake_CB_class', 'real_CA_class', 'real_CB_class', 'real_CA_mask_vis', 'real_CA_vis', 'real_CA_edge_vis']
    ...
    def set_input(self, input):
        ...
        self.real_CA = input['CA'].to(self.device)
        self.real_CB = input['CB'].to(self.device)
    ...
    def forward(self):
        ...
        # get fake_B with alpha channel
        self.fake_B = self.real_A * (self.fake_B_A * -1.0 + 1.0) + self.fake_B_RGB * self.fake_B_A
        # alpha channel for vis
        self.fake_B_A = self.fake_B_A.unsqueeze(0)
        self.fake_CA = self.netGc(self.fake_B.detach())  # Gc(fake_B)
        self.fake_CA_real_A = self.netGc(self.real_A)  # Gc(real_A)
        self.fake_CB_real_B = self.netGc(self.real_B)  # Gc(real_B)
        self.fake_CA_fake_vis = self.fake_CA[:,3,:,:].unsqueeze(0)
        self.fake_CA_fake_edge_vis = self.fake_CA[:,4,:].unsqueeze(0)
        self.real_CA_fake_vis = self.real_CA[:,3,:,:].unsqueeze(0)
        self.real_CA_fake_edge_vis = self.real_CA[:,4,:,:].unsqueeze(0)
        self.fake_CB_real_B_fake_vis = self.fake_CB_real_B[:,3,:,:].unsqueeze(0)
        self.fake_CB_real_B_fake_edge_vis = self.fake_CB_real_B[:,4,:,:].unsqueeze(0)
        self.real_CA_class = self.real_CA[:,0:3,:,:]
        self.real_CB_class = self.real_CB[:,0:3,:,:]
        self.fake_CA_class = self.fake_CA[:,0:3,:,:]
        self.fake_CB_class = self.fake_CB_real_B[:,0:3,:,:]
        #if self.real_C_real_B is None:
        #    self.real_C_real_B = self.fake_C_real_B.new_full(self.fake_C_real_B.size(), -1.0)
        self.real_CA_vis = self.real_CA[:,3,:,:].unsqueeze(0)
        self.real_CA_edge_vis = self.real_CA[:,4,:,:].unsqueeze(0)
    ...
    def backward_Dc(self):
        """Calculate GAN loss for the discriminator"""
        # Fake; stop backprop to the generator by detaching fake_B
        fake_BC = torch.cat((self.fake_B, self.fake_CA), 1)  # we use conditional GANs; we need to feed both input and output to the discriminator
        pred_fake = self.netDc(fake_BC.detach())
        self.loss_Dc_fake = self.criterionGAN(pred_fake, False)
        # Real
        real_BC = torch.cat((self.fake_B, self.real_CA), 1)
        pred_real = self.netDc(real_BC.detach())
        self.loss_Dc_real = self.criterionGAN(pred_real, True)
        
        # Fake; stop backprop to the generator by detaching fake_B
        fake_BC = torch.cat((self.real_B, self.fake_CB_real_B), 1)  # we use conditional GANs; we need to feed both input and output to the discriminator
        pred_fake = self.netDc(fake_BC.detach())
        self.loss_Dc_fake_B = self.criterionGAN(pred_fake, False)
        # Real
        real_BC = torch.cat((self.real_B, self.real_CB), 1)
        pred_real = self.netDc(real_BC.detach())
    ...
    def backward_Gb(self):
        ...
        # Third, Gc(Gb(A)) -> min
        fake_CA = self.netGc(self.fake_B)  # Gc(B)
        fake_CA_fake = fake_CA[:,3:5,:,:]
        fake_CA_class = fake_CA[:,0:3,:,:]
        #print(fake_C.mean())
        self.loss_Gb_M = torch.abs(fake_CA_fake.mean() - -1) * self.opt.lambda_M
        # Fourth, class labels should be the same as for real_A
        self.loss_Gb_S = self.criterionL1(fake_CA_class, self.real_CA[:,0:3,:,:]) * self.opt.lambda_S
        # combine loss and calculate gradients
        self.loss_Gb = self.loss_Gb_GAN + self.loss_Gb_L1 + self.loss_Gb_M + self.loss_Gb_S
    ...
    def backward_Gc(self):
        ...
        fake_BC = torch.cat((self.fake_B, self.fake_CA), 1)
        pred_fake = self.netDc(fake_BC.detach())
        self.loss_Gc_GAN = self.criterionGAN(pred_fake, True)
        # Second, Gc(B) = CA
        real_CA = self.real_CA.clone().detach()
        # there should be no class labels for spilces in fake_CA
        mask = self.real_CA[:,3,:,:].clone().detach()
        mask = ((mask + 1.0) / 2.0) * -1.0 + 1.0
        real_CA[:,0:3,:,:] = (((real_CA[:,0:3,:,:] + 1.0)/2.0 * mask) - 0.5) / 0.5
        self.real_CA_mask_vis = real_CA[:,0:3,:,:]
        self.loss_Gc_L1 = networks.balancedL1(real_CA, self.fake_CA, weights=[0.24, 0.24, 0.24, 0.6, 0.24]) * self.opt.lambda_L1_Gc

        # Third, Gc(real_A) = CA
        self.loss_Gc_real_A_L1 = networks.balancedL1(real_CA, self.fake_CA_real_A, weights=[0.24, 0.24, 0.24, 0.6, 0.24]) * self.opt.lambda_L1_Gc


        # Fourth, Gc(real_B) = 0
        #self.loss_Gc_real_B_L1 = self.criterionL1(self.fake_CB_real_B, self.real_CB) * self.opt.lambda_L1_Gc
        #self.loss_Gc_real_B_L1 = self.criterionL1(self.fake_CB_real_B[:,0:3,:,:], self.real_CB[:,0:3,:,:]) * self.opt.lambda_L1_Gc 
        #self.loss_Gc_real_B_L1 *= 0.55
        self.loss_Gc_real_B_L1 = networks.balancedL1(self.real_CB, self.fake_CB_real_B, weights=[0.24, 0.24, 0.24, 0.6, 0.24]) * self.opt.lambda_L1_Gc
        #print('self.loss_Gc_real_B_L1 = {}'.format(self.loss_Gc_real_B_L1))
        # combine loss and calculate gradients
        #self.loss_Gc = self.loss_Gc_GAN + self.loss_Gc_L1 + self.loss_Gc_real_B_L1 + self.loss_Gc_real_A_L1 + self.loss_Gc_real_B_L1_class
