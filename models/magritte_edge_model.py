...
class MAGritteEdgeModel(BaseModel):
    ...
    def modify_commandline_options(parser, is_train=True):
        ...
        if is_train:
            ...
            parser.add_argument('--lambda_L1_Gc_fake', type=float, default=1.0, help='weight for L1 loss')
    ...
    def __init__(self, opt):
        """Initialize the MAGritteEdgeModel class.
        ...
        self.visual_names = ['real_A', 'fake_B', 'fake_B_RGB', 'fake_B_A', 'real_B', 'fake_C_vis', 'fake_C_edge_vis', 'fake_C_real_B_vis', 'fake_C_real_B_edge_vis', 'fake_C_real_A_vis', 'fake_C_real_A_edge_vis', 'real_C_vis', 'real_C_edge_vis']
        ...
        self.netGc = networks.define_G(opt.input_nc, opt.output_nc, int(opt.ngf/1), opt.netG, opt.norm,
                                      not opt.no_dropout, opt.init_type, opt.init_gain, self.gpu_ids)
    ...
    def forward(self):
        ...
        self.fake_C_vis = self.fake_C[:,0,:,:].unsqueeze(0)
        self.fake_C_edge_vis = self.fake_C[:,1,:,:].unsqueeze(0)
        self.fake_C_real_A = self.netGc(self.real_A)  # Gc(real_A)
        self.fake_C_real_A_vis = self.fake_C_real_A[:,0,:,:].unsqueeze(0)
        self.fake_C_real_A_edge_vis = self.fake_C_real_A[:,1,:,:].unsqueeze(0)
        self.fake_C_real_B = self.netGc(self.real_B)  # Gc(real_B)
        self.fake_C_real_B_vis = self.fake_C_real_B[:,0,:,:].unsqueeze(0)
        self.fake_C_real_B_edge_vis = self.fake_C_real_B[:,1,:,:].unsqueeze(0)
        if self.real_C_real_B is None:
            self.real_C_real_B = self.fake_C_real_B.new_full(self.fake_C_real_B.size(), -1.0)
        
        self.real_C_vis = self.real_C[:,0,:,:].unsqueeze(0)
        self.real_C_edge_vis = self.real_C[:,1,:,:].unsqueeze(0)
    ...
    def backward_Gb(self):
        ...
        # self.loss_Gb = self.loss_Gb_L1 + self.loss_Gb_M
        # self.loss_Gb = self.loss_Gb_GAN
    ...
    def backward_Gc(self):
        ...
        # Second, Gc(B) = C
        #self.loss_Gc_L1 = self.criterionL1(self.fake_C, self.real_C) * self.opt.lambda_L1_Gc * 1000.0
        C_weights = [0.2, 0.8]
        self.loss_Gc_L1 = networks.balancedL1(self.real_C, self.fake_C, weights=C_weights) * self.opt.lambda_L1_Gc_fake
        
        # Third, Gc(real_A) = C
        #self.loss_Gc_real_A_L1 = self.criterionL1(self.fake_C_real_A, self.real_C) * self.opt.lambda_L1_Gc * 1000.0
        self.loss_Gc_real_A_L1 = networks.balancedL1(self.real_C, self.fake_C_real_A, weights=C_weights) * self.opt.lambda_L1_Gc
        
        # Fourth, Gc(real_B) = 0
        #self.loss_Gc_real_B_L1 = self.criterionL1(self.fake_C_real_B, self.real_C_real_B) * self.opt.lambda_L1_Gc
        self.loss_Gc_real_B_L1 =  networks.balancedL1(self.real_C_real_B, self.fake_C_real_B, weights=C_weights) * self.opt.lambda_L1_Gc
        #self.loss_Gc_real_B_L1 = 0
