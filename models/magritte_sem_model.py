...
class MAGritteSemModel(BaseModel):
    ...
    def __init__(self, opt):
        """Initialize the MAGritteSemModel class.
        ...
        self.visual_names = ['real_A', 'fake_B', 'fake_B_RGB', 'fake_B_A', 'real_B', 'fake_CA_fake', 'fake_CB_real_B_fake', 'fake_CA_class', 'fake_CB_class', 'real_CA_class', 'real_CB_class']
    ...
    def forward(self):
        ...
        self.fake_CA_fake = self.fake_CA[:,3,:,:].unsqueeze(0)
        self.real_CA_fake = self.real_CA[:,3,:,:].unsqueeze(0)
        self.fake_CB_real_B_fake = self.fake_CB_real_B[:,3,:,:].unsqueeze(0)
    ...
    def backward_Gc(self):
        ...
        # there should be no class labels in fake_CA
        real_CA[:,0:3,:,:] = -1.0
        self.loss_Gc_L1 = self.criterionL1(self.fake_CA, real_CA) * self.opt.lambda_L1_Gc * 0.5
        # Third, Gc(real_A) = CA
        self.loss_Gc_real_A_L1 = self.criterionL1(self.fake_CA_real_A[:,3,:,:], self.real_CA[:,3,:,:]) * self.opt.lambda_L1_Gc
        self.loss_Gc_real_A_L1 = 0
        # Fourth, Gc(real_B) = 0
        self.loss_Gc_real_B_L1 = self.criterionL1(self.fake_CB_real_B, self.real_CB) * self.opt.lambda_L1_Gc
        #self.loss_Gc_real_B_L1_class = self.criterionL1(self.fake_CB_real_B[:,0:3,:,:], self.real_CB[:,0:3,:,:]) * self.opt.lambda_L1_Gc 
        ...
        self.loss_Gc = self.loss_Gc_GAN + self.loss_Gc_L1 + self.loss_Gc_real_B_L1
